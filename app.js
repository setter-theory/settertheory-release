...
function buildSetterIqRadarChartPdf(breakdown){
  ...
  return `<div class="setterIqRadar pdfSetterIqRadar" aria-label="Setter IQ 5項目レーダーチャート">
    <svg viewBox="0 0 340 292" role="img" aria-label="配球、多様性、ミドル、勝負所、安定性の評価">
      <g class="radarGrid"><polygon points="${polygon(1)}"/><polygon points="${polygon(.75)}"/><polygon points="${polygon(.5)}"/><polygon points="${polygon(.25)}"/>${axes}</g>
      <polygon class="radarData" points="${dataPoints}"/>
      <g class="radarDots">${items.map((item,i)=>{const [x,y]=point(i,Math.max(0,Math.min(20,item.value))/20);return `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="4"/>`;}).join('')}</g>
      <g class="radarLabels">${labels}</g>
...
function printMatchPdfReport(){
  ...
  const pdfSetterLeadCards=(reportSetters.length>1 ? reportSetters : [null]).map((n,i)=>{
    ...
    const title=n===null?'セッター分析':`セッター分析${i+1}　${esc(n)}番 ${esc(a.name||'')}`;
    const advice=n===null ? aquilaAdvice : getAquilaAdviceForSetter(n);
    const tossItems=['レフト','センター','ライト','バック','ツー'].map(k=>({label:k,value:a.counts[k]||0}));
    const tossTotal=tossItems.reduce((sum,x)=>sum+x.value,0);
    const tossLegend=tossItems.map(x=>`<div><span>${esc(x.label)}</span><b>${x.value}本</b></div>`).join('');
    const rotRows=(a.rotationRows||[]).map(x=>`<div class="pdfRotationRow"><span>${esc(x.rot)}</span><b>${x.total||0}本</b><small>${x.rate||0}%</small></div>`).join('');
    return `<section class="pdfSetterAnalysisCard"><div class="pdfSetterAnalysisHead"><div><div class="title">${title}</div><div class="pdfSetterScore"><b>${score||'--'}</b><span>/100</span><small>${score?rank.label:'NO DATA'}</small></div></div><div class="pdfAdviceInline"><strong>Aquila Advice</strong><ul>${advice.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div></div><div class="pdfSetterSubgrid"><div class="pdfSetterSubcard"><h3>能力バランス</h3><div class="pdfSetterSubbody radarBody">${buildSetterIqRadarChartPdf(breakdown)}</div></div><div class="pdfSetterSubcard"><h3>トス配分</h3><div class="pdfSetterSubbody tossBody"><div class="pdfDonut" style="background:${donutStyle(tossItems)}"><span>${tossTotal}<small>本</small></span></div><div class="pdfTossLegend">${tossLegend}</div></div></div><div class="pdfSetterSubcard"><h3>ローテーション別トス配分</h3><div class="pdfSetterSubbody rotationBody">${rotRows||'<div class="emptyMini">記録なし</div>'}</div></div></div></section>`;
  }).join('');
  ...
  <style>
    ...
    *{box-sizing:border-box;}
    ...
    .pdfLead{display:grid;grid-template-columns:1fr;gap:12px;margin:10px 0 12px;align-items:stretch}.pdfSetterLeadPerson{display:grid;grid-template-columns:1fr;gap:8px;break-inside:avoid;page-break-inside:avoid}.pdfIqCard,.pdfAdviceCard{border:1px solid #cbd5e1;border-radius:14px;padding:11px;background:linear-gradient(180deg,#f8fafc,#fff);break-inside:avoid}.pdfIqCard .title,.pdfAdviceCard .title{font-size:12px;font-weight:950;color:#1e3a8a;margin-bottom:6px}
    .pdfSetterAnalysisCard{border:1.5px solid #93c5fd;border-radius:16px;padding:6px;background:linear-gradient(180deg,#eff6ff,#fff);break-inside:avoid;page-break-inside:avoid}.pdfSetterAnalysisHead{display:grid;grid-template-columns:180px 1fr;gap:8px;align-items:start;margin-bottom:5px}.pdfSetterAnalysisHead .title{font-size:14px;font-weight:1000;color:#1e3a8a}.pdfSetterScore{display:flex;align-items:baseline;gap:3px}.pdfSetterScore b{font-size:30px;color:#2563eb}.pdfSetterScore span{font-size:11px}.pdfSetterScore small{margin-left:6px;background:#0f172a;color:#fbbf24;border-radius:999px;padding:3px 7px;font-weight:900}.pdfAdviceInline{font-size:9px;line-height:1.35}.pdfAdviceInline strong{color:#1e3a8a}.pdfAdviceInline ul{margin:3px 0 0;padding-left:15px}.pdfSetterSubgrid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:4px;width:calc(100% + 4px);margin:0 -2px -2px;align-items:stretch}.pdfSetterSubcard{min-width:0;height:216px;border:1px solid #bfdbfe;border-radius:12px;overflow:hidden;background:#fff;display:flex;flex-direction:column}.pdfSetterSubcard h3{margin:0;padding:6px 6px;background:#1e3a8a;color:#fff;font-size:10px;text-align:center;flex:0 0 auto}.pdfSetterSubbody{padding:4px;box-sizing:border-box;min-height:0;height:auto;flex:1 1 auto}.pdfSetterSubbody.radarBody{display:flex;align-items:center;justify-content:center}.pdfSetterSubbody.radarBody .setterIqRadar{border:0;background:transparent;padding:0;width:100%}.pdfSetterSubbody.radarBody .radarLabels text{font-size:11px;font-weight:1000;fill:#334155}.pdfSetterSubbody.radarBody .radarLabels tspan+tspan{font-size:10px;font-weight:1000;fill:#1d4ed8}.pdfSetterSubbody.radarBody .setterIqRadar svg{display:block;width:92%;height:170px;overflow:visible;margin:auto}.tossBody{display:grid;grid-template-columns:115px 1fr;gap:6px;align-items:center}.pdfDonut{width:105px;height:105px;border-radius:50%;display:grid;place-items:center;position:relative;margin:auto}.pdfDonut:after{content:'';position:absolute;width:58px;height:58px;border-radius:50%;background:#fff}.pdfDonut span{position:relative;z-index:1;font-size:18px;font-weight:1000;color:#0f172a}.pdfDonut small{font-size:8px;margin-left:1px}.pdfTossLegend{display:grid;gap:4px;font-size:8px}.pdfTossLegend div{display:flex;justify-content:space-between;gap:4px;border-bottom:1px solid #e2e8f0;padding-bottom:2px}.rotationBody{display:grid;grid-template-columns:1fr;gap:4px;align-content:start}.pdfRotationRow{display:grid;grid-template-columns:32px 1fr 38px;align-items:center;gap:4px;border-radius:7px;background:#eff6ff;padding:5px 6px;font-size:9px}.pdfRotationRow span{font-weight:1000;color:#1e3a8a}.pdfRotationRow b{text-align:right}.pdfRotationRow small{text-align:right;color:#64748b}.emptyMini{display:grid;place-items:center;height:100%;font-size:10px;color:#64748b}.pdfIqCard .score{font-size:36px;font-weight:1000;color:#2563eb;line-height:1}.pdfIqCard .score small{font-size:13px;color:#64748b}.pdfIqCard .rank{display:inline-block;margin-top:5px;border-radius:999px;padding:4px 9px;background:#0f172a;color:#fbbf24;font-size:9px;font-weight:950}.pdfIqScoreRow{display:grid;grid-template-columns:96px 1fr;gap:10px;align-items:center}.pdfIqBreakdown{display:grid;grid-template-columns:1fr 1fr;gap:4px;margin-top:6px;font-size:8px;color:#475569}.pdfIqBreakdown span{background:#e2e8f0;border-radius:6px;padding:4px}.pdfAdviceCard ul{margin:0;padding-left:17px;font-size:10px;line-height:1.55}.pdfAdviceCard li+li{margin-top:4px}
